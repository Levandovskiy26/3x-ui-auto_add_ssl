...
# Function to generate SSL certificate with domain
gen_ssl_cert_with_domain() {
    local domain=$1
    openssl req -x509 -newkey rsa:4096 -nodes -sha256 -keyout /etc/ssl/private/3x-ui-private.key -out /etc/ssl/certs/3x-ui-public.key -days 3650 -subj "/CN=$domain"
}
...
gen_ssl_cert_with_domain "$domain"
get_last_id
execute_sql_inserts
